...
const audioChat = document.getElementById("chatAlertAudio");
const audioNoti = document.getElementById("notiSonido");

// Variable para controlar si el sonido ya está permitido
...
document.addEventListener('click', () => {
  if (!sonidoPermitido) {
    audioNoti.play().then(() => {
      ...
      console.log("Sonido activado tras interacción del usuario.");
    }).catch(() => {
      // No se pudo reproducir aún, se intentará en la próxima interacción
    });
  }
}, { once: true });
...
function mostrarNotificacion(titulo, cuerpo) {
  if (Notification.permission === "granted") {
    new Notification(titulo, { body: cuerpo, icon: "/icono.png" });
  }
}
...
const Pedidos = {
 ...
 obtenerUbicacion() {
      ...
      function (error) {
        switch (error.code) {
          case error.PERMISSION_DENIED:
            status.textContent = "Permiso denegado para acceder a la ubicación. Por favor, habilita el permiso en tu navegador.";
            break;
          case error.POSITION_UNAVAILABLE:
            status.textContent = "La ubicación no está disponible en este momento.";
            break;
          case error.TIMEOUT:
            status.textContent = "Se agotó el tiempo para obtener la ubicación. Intenta de nuevo.";
            break;
          default:
  ...
  // Chat de pedido (con repartidor) con notificaciones
  ...
  db.collection("pedidos")
    ...
    .onSnapshot((snapshot) => {
      ...
      snapshot.forEach((doc) => {
        ...
        mensajesDiv.innerHTML += `<div class="${claseMensaje} mb-1"><b>${Utils.escapeHtml(m.nombreEmisor)}:</b> ${Utils.escapeHtml(m.texto)}</div>`;
      });
      ...
      if (hayNuevoMensaje) {
        ...
        mostrarNotificacion("Nuevo mensaje en el chat", "Tienes un nuevo mensaje del repartidor.");
      }
    });
},
      ...
      .onSnapshot(snapshot => {
        ...
        if (hayNoLeido) {
          ...
          if (mensajeNuevo && document.getElementById("chatAdminBox").classList.contains("hidden")) {
            try { audioChat.play(); } catch (e) { }
          }
          mostrarNotificacion("Nuevo mensaje en el chat", "Tienes un nuevo mensaje del administrador.");
        } else {
    ...
    if (ultimoMs > 0 && currentUser) {
      ...
      try {
        await db.collection("usuarios").doc(currentUser.uid).update({
          chatAdminUltLeido: ultimoMs
        });
      } catch (error) {
        console.error("Error actualizando último leído en Firestore:", error);
      }
    }
  },
  async enviarMensajeChatAdmin() {
    ...
    try {
      await db.collection("chats_clientes")
        .doc(currentUser.uid)
        .collection("mensajes")
        .add({
          texto,
...
function escucharCambiosPedidos() {
  if (!currentUser) return;
  db.collection("pedidos")
    ...
    .onSnapshot((snapshot) => {
      snapshot.docChanges().forEach((change) => {
        if (change.type === "modified") {
          ...
          if (sonidoPermitido) {
            audioNoti.play().catch(() => {});
          }
          mostrarNotificacion(
            "Estado del pedido actualizado",
            `Tu pedido en ${Utils.formatearTipoServicio(pedido.tipoServicio)} está ahora "${pedido.estado}"`
          );
          Pedidos.cargarPedidos();
        }
      });
...
// Llamar esta función después de autenticar usuario
function iniciarNotificacionesCliente() {
  escucharCambiosPedidos();
  ChatAdmin.escucharChatAdmin();
}
